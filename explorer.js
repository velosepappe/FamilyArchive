...
function displayPhoto(photoDetails){
	...
	url = "file:///";
	url += currentPath==""?mainDirectory:currentPath;
	  ...
	  .done(function( data ) {
		...
		$.each(list, function(index, location){
			...
			if(index > 0){
				...
				else if(location[5] > 10){
					icon.scaledSize= new google.maps.Size(32, 32);
				}
				else if(location[5] > 1){
					icon.scaledSize= new google.maps.Size(24, 24);
				}
				else if(location[5] == 1){
					icon.scaledSize= new google.maps.Size(16, 16);
				}
				else{
					icon.scaledSize= new google.maps.Size(12, 12);
				}
				markers[location[0]].setIcon(icon);
			}
			if(photoLocation){
